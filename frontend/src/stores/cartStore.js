...
export const useCartStore = defineStore('cart', () => {
  ...
  const totalPuntos = computed(() => items.value.reduce((total, item) => total + item.producto.precioPuntos * item.cantidad, 0));
  ...
  async function agregarAlCarrito(productoId, event, cantidad = 1) {
    // ... (la lógica de la animación "fly-to-cart" puede permanecer aquí si lo deseas, es visual)
    if (event) {
    ...
    try {
      ...
      Swal.fire({ icon: "success", title: "¡Agregado!", text: response.data.message, toast: true, position: "top-end", showConfirmButton: false, timer: 2000, timerProgressBar: true });
    } catch (error) {
  ...
  async function eliminarDelCarrito(productoId) {
    try {
      ...
      await fetchCarrito();
      Swal.fire({ icon: "info", title: "Eliminado", text: response.data.message, toast: true, position: "top-end", showConfirmButton: false, timer: 2000 });
    } catch (error) {
  ...
  async function procesarCanje() {
    ...
    const { isConfirmed } = await Swal.fire({ title: "¿Confirmar Canje?", html: `Se canjearán <strong>${totalItems.value} productos</strong> por <strong>${totalPuntos.value} puntos</strong>.`, icon: "question", showCancelButton: true, confirmButtonText: "Sí, ¡canjear!", cancelButtonText: "Cancelar" });
  ...
  // 4. Exponemos todo para que los componentes puedan usarlo
  return {
    items,
    loading,
    totalItems,
