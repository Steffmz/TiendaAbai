  ...
  setup() {
    ...
    const cargarProductos = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.categoria) {
            categoriaNombre.value = data.categoria.nombre;
          }
    ...
    const editarProducto = (producto) => {
      ...
      if (producto.imagenUrl) {
        previewImage.value = `http://localhost:3000${producto.imagenUrl}`;
      } else {
        previewImage.value = null;
      }
      mostrarModal.value = true;
    ...
    const guardarProducto = async () => {
      try {
        ...
        if (res.ok) {
          alert(editando.value ? 'Producto actualizado exitosamente' : 'Producto creado exitosamente');
          mostrarModal.value = false;
