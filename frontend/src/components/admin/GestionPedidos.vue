  ...
  <div class="max-w-7xl w-full mx-auto">
    ...
    <div v-if="!loading && totalPages > 1" class="flex flex-col items-center justify-center mt-4">
        ...
        <div class="flex items-center mt-2 space-x-1">
            <button @click="prevPage" :disabled="currentPage === 1" class="w-8 h-8 flex items-center justify-center rounded-md bg-[#fffef9] border border-gray-200 text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">←</button>
            <button v-for="pagina in paginasVisibles" :key="pagina" @click="goToPage(pagina)" :class="['w-8 h-8 flex items-center justify-center rounded-md border font-medium', currentPage === pagina ? 'bg-blue-500 text-white border-blue-500' : 'bg-[#fffef9] border-gray-200 text-gray-600 hover:bg-gray-100', pagina === '...' ? 'cursor-default' : '']">{{ pagina }}</button>
            <button @click="nextPage" :disabled="currentPage === totalPages" class="w-8 h-8 flex items-center justify-center rounded-md bg-[#fffef9] border border-gray-200 text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">→</button>
        </div>
    </div>
    <div v-if="selectedPedido" class="modal-overlay" @click.self="closeDetailsModal">
      <div class="modal-content">
        ...
        <ul class="product-list">
          <li v-for="detalle in selectedPedido.detalles" :key="detalle.id">{{ detalle.cantidad }} x {{ detalle.producto.nombre }} ({{ detalle.puntosUnitarios }} pts c/u)</li>
        </ul>
        <div class="modal-actions"><button @click="closeDetailsModal" class="btn btn-primary">Cerrar</button></div>
      </div>
    </div>
  </div>
</template>
...
const getStatusClass = (estado) => {
  const classes = { Pendiente: 'warning', Aprobado: 'success', Enviado: 'info', Entregado: 'success-dark', Cancelado: 'danger', Rechazado: 'danger' };
...
const paginasVisibles = computed(() => {
    ...
    for (let i = Math.max(2, actual - rango); i <= Math.min(total - 1, actual + rango); i++) paginas.push(i);
...
.details-subtitle { font-size: 1.1rem; margin-top: 1.5rem; margin-bottom: 0.5rem; border-bottom: 1px solid var(--border); padding-bottom: 0.5rem; font-weight: 600; }
