<template>
  <div class="max-w-7xl w-full mx-auto flex flex-col h-full">
    ...
    <div class="flex-grow overflow-y-auto pb-4">
      <div class="hidden md:block rounded-xl border border-gray-200 shadow-sm w-full overflow-hidden">
          ...
          <tbody>
            <tr v-if="loading">
              <td colspan="4" class="p-0">
                <div v-for="i in 4" :key="i" class="flex items-center p-4 gap-4 border-b border-[var(--border)]">
            ...
            <tr v-else v-for="categoria in categoriasPaginadas" :key="categoria.id">
              ...
              <td class="actions-cell">
                <button @click="abrirModalEditar(categoria)" class="bg-blue-100 text-black hover:bg-blue-200 px-3 py-1 rounded-md text-sm font-medium transition-colors duration-150">Editar</button>
                <button @click="irAProductos(categoria.id)" class="bg-green-100 text-black hover:bg-green-200 px-3 py-1 rounded-md text-sm font-medium transition-colors duration-150">Catálogo</button>
                ...
                <button @click="confirmarEliminar(categoria)" class="bg-red-100 text-black hover:bg-red-200 px-3 py-1 rounded-md text-sm font-medium transition-colors duration-150">Eliminar</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="cards-container md:hidden">
        ...
        <div v-else v-for="categoria in categoriasPaginadas" :key="categoria.id" class="card">
          <div class="card-header">
            <img :src="categoria.imagenUrl ? `${API_BASE_URL}${categoria.imagenUrl}` : placeholder" class="card-image" />
          ...
          <div class="card-actions">
            <button @click="abrirModalEditar(categoria)" class="bg-blue-100 text-black hover:bg-blue-200">Editar</button>
            <button @click="irAProductos(categoria.id)" class="bg-green-100 text-black hover:bg-green-200">Catálogo</button>
            <button @click="cambioEstado(categoria)" :class="[categoria.activo ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800']">
              {{ categoria.activo ? "Desactivar" : "Activar" }}
            </button>
            <button @click="confirmarEliminar(categoria)" class="bg-red-100 text-black hover:bg-red-200">Eliminar</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="mostrarModal" class="modal-overlay" @click.self="cerrarModal">
      <div class="modal-content">
        ...
        <form @submit.prevent="guardarCategoria">
          <div class="form-group"><label>Nombre</label><input v-model="form.nombre" type="text" required placeholder="Nombre de la categoría" /></div>
          <div class="form-group"><label>Imagen</label><input type="file" @change="handleImageUpload" accept="image/*" /><div v-if="previewImage" class="mt-4 flex justify-center"><img :src="previewImage" class="w-24 h-24 rounded-lg object-cover border" alt="Preview" /></div></div>
          <div class="modal-actions"><button type="button" @click="cerrarModal" class="btn btn-secondary">Cancelar</button><button type="submit" class="btn btn-primary">{{ editando ? 'Actualizar' : 'Crear' }}</button></div>
        </form>
      </div>
    </div>
  </div>
